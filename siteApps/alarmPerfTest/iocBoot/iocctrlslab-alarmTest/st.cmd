...
epicsEnvSet("LOCATION",  "ctrlslab")
epicsEnvSet("STARTUP", "$(TOP)")
epicsEnvSet("ST_CMD", "st.cmd")
epicsEnvSet("EPICS_TIMEZONE", "EPICS_TZ")
epicsEnvSet("WIKI", "")
#- 
epicsEnvSet("IOCNAME", "ctrlslab-alarmTest")
epicsEnvSet("IOC", "iocctrlslab-alarmTest")
#-
epicsEnvSet("P", "ctrlslab")
epicsEnvSet("F", "F")
epicsEnvSet("R", "BBBB:")

dbLoadDatabase "dbd/alarmTest.dbd"
alarmTest_registerRecordDeviceDriver pdbbase
...
#- access control list
#-asSetFilename("$(DB_TOP)/access_securityctrlslab-alarmTest.acf")
...
#iocshLoad("$(IOCSH_LOCAL_TOP)/alarmTest.iocsh", "P=$(P),R=$(R),DATABASE_TOP=$(DB_TOP),PORT=$(PORT1),IPADDR=$(PORT1_IP),IPPORT=$(PORT1_PORT)")

iocshLoad("$(IOCSH_LOCAL_TOP)/alarmTest.iocsh", "P=$(P),DATABASE_TOP=$(DB_TOP),F=$(F)")
