...
static int sendCmd(llrfInfo *pllrfInfo, uint32 cmd, int32 attr, int32 value)
{
	...
	switch( cmd ) {
		...
		case CMD_ACK_INIT :
            ...
            sendData.value[ATTR_PAC_MODE] = pllrfInfo->bPacMode;
            sendData.value[ATTR_PULSE_LENGTH] = pllrfInfo->iPulseLength;
			break;
		case CMD_ATTR :
...
void reassemble_mean_data(char *buf)
{
    ...
    meanData.normal_error = *((unsigned char *)pos);
    pos += sizeof(unsigned char);

    meanData.netStat = *((unsigned char *)pos);
    pos += sizeof(unsigned char);
}
...
void reassemble_raw_data(char *buf)
{
    ...
    rawData.grp_error = *((unsigned char *)pos);
    pos += sizeof(unsigned char);

    rawData.netStat = *((unsigned char *)pos);
    pos += sizeof(unsigned char);
...
STATIC gtrStatus llrfpulseLength(gtrPvt pvt, double value)
{
	llrfInfo *pllrfInfo = (llrfInfo *)pvt;

   	pllrfInfo->iPulseLength = (int32)(value*1000);

	printf("PULSE:LENGTH -> %lf\n", value);

	sendCmd(pllrfInfo, CMD_ATTR, ATTR_PULSE_LENGTH, pllrfInfo->iPulseLength);
...
STATIC gtrStatus llrfaoFunction(long parm, gtrPvt pvt, double value, int index)
{
	...
	switch(parm) {
		...
		case ampHighLimit:
			...
			break;
        case pulseLength:
            status = llrfpulseLength(pvt, value);
            break;
	}
	return (status);
}
...
STATIC gtrStatus llrfPacMode(gtrPvt pvt, int value)
{
	llrfInfo *pllrfInfo = (llrfInfo *)pvt;

	pllrfInfo->bPacMode = value;

	sendCmd(pllrfInfo, CMD_ATTR, ATTR_PAC_MODE, pllrfInfo->bPacMode);
...
static const iocshArg *llrfConfigArgs[] = { &llrfConfigArg0, &llrfConfigArg1, &llrfConfigArg2, &llrfConfigArg3 };
