...
void QSimpleShape::paintEvent (QPaintEvent* e)
{
   ...
   switch (this->getShape ()) {
	 ...
	 case valve_h:
         polygon[0] = QPoint (rect.left ()+5, rect.top ());
         polygon[1] = QPoint (rect.left ()+5, rect.bottom ());
         polygon[2] = QPoint (rect.right ()-5, rect.top ());
         polygon[3] = QPoint (rect.right ()-5, rect.bottom ());
         polygon[4] = polygon[0];
         painter.drawPolygon (polygon, 5);
...

		{
         ...
         //painter.drawEllipse(circleX, circleY, circleDiameter*0.4, circleDiameter*0.4);
         painter.drawEllipse(rect.center(), circleDiameter*0.2, circleDiameter*0.2);
		}
...
// Define the rectangle dimensions
        ...
        // Calculate the circle dimensions (inscribed in the rectangle)
        int circleDiameter = qMin(rectWidth, rectHeight);
        int circleX = rectX + (rectWidth - circleDiameter) / 2;
        int circleY = rectY + (rectHeight - circleDiameter) / 2;
	 ...
	 case con_valve_h:
		polygon[0] = QPoint(rect.width()*0.5, 0);
		polygon[1] = QPoint(rect.width()*0.5, rect.height()*0.75 );
		polygon[2] = QPoint(0,  rect.height()*0.5);
		polygon[3] = QPoint(rect.left(),  rect.bottom());
		polygon[4] = QPoint(rect.width(), rect.height()*0.5);
		polygon[5] = QPoint(rect.right(), rect.bottom());
		polygon[6] = polygon[1];
		painter.drawPolygon (polygon, 7);
	 ...
	 case rectcorner:
		...
		{
		 line_1.setP1(QPoint(rect.left(), rect.top()));
		 line_1.setP2(QPoint(rect.width()*0.25, rect.top()));
		 painter.drawLine( line_1 );

		 line_1.setP1(QPoint(rect.width()*0.75, rect.top()));
		 line_1.setP2(QPoint(rect.right(), rect.top()));
		 painter.drawLine( line_1 );

		 line_1.setP1(QPoint(rect.right(), rect.top()));
		 line_1.setP2(QPoint(rect.right(), rect.height()*0.25));
		 painter.drawLine( line_1 );

		 line_1.setP1(QPoint(rect.right(), rect.height()*0.75));
		 line_1.setP2(QPoint(rect.right(), rect.bottom()));
		 painter.drawLine( line_1 );

		 line_1.setP1(QPoint(rect.right(), rect.bottom()));
		 line_1.setP2(QPoint(rect.width()*0.75, rect.bottom()));
		 painter.drawLine( line_1 );

		 line_1.setP1(QPoint(rect.left(), rect.bottom()));
		 line_1.setP2(QPoint(rect.width()*0.25, rect.bottom()));
		 painter.drawLine( line_1 );

		 line_1.setP1(QPoint(rect.left(), rect.bottom()));
		 line_1.setP2(QPoint(rect.left(), rect.height()*0.75));
		 painter.drawLine( line_1 );

		 line_1.setP1(QPoint(rect.left(), rect.top()));
		 line_1.setP2(QPoint(rect.left(), rect.height()*0.25));
		 pen.setWidth(ew);
		 painter.setPen(pen);
		 painter.drawLine( line_1 );
		}
#else
		{
         p = 0;
         polygon[p++]  = QPoint (rect.left(), rect.top());
         polygon[p++]  = QPoint(rect.width()*0.25, rect.top());
         polygon[p++]  = QPoint(rect.width()*0.25, rect.top()+cornerWidth);
         polygon[p++]  = QPoint (rect.left()+cornerWidth, rect.top()+cornerWidth);
         polygon[p++]  = QPoint (rect.left()+cornerWidth, rect.height()*0.25);
         polygon[p++]  = QPoint (rect.left(), rect.height()*0.25);
         polygon[p++]  = polygon[0];            // close loop
         painter.drawPolygon (polygon, p);

         p = 0;
         polygon[p++]  = QPoint (rect.left(), rect.bottom());
         polygon[p++]  = QPoint(rect.width()*0.25, rect.bottom());
         polygon[p++]  = QPoint(rect.width()*0.25, rect.bottom()-cornerWidth);
         polygon[p++]  = QPoint (rect.left()+cornerWidth, rect.bottom()-cornerWidth);
         polygon[p++]  = QPoint (rect.left()+cornerWidth, rect.height()*0.75);
         polygon[p++]  = QPoint (rect.left(), rect.height()*0.75);
         polygon[p++]  = polygon[0];            // close loop
         painter.drawPolygon (polygon, p);

         p = 0;
         polygon[p++]  = QPoint(rect.right(), rect.top());
         polygon[p++]  = QPoint(rect.width()*0.75, rect.top());
         polygon[p++]  = QPoint(rect.width()*0.75, rect.top()+cornerWidth);
         polygon[p++]  = QPoint (rect.right()-cornerWidth, rect.top()+cornerWidth);
         polygon[p++]  = QPoint (rect.right()-cornerWidth, rect.height()*0.25);
         polygon[p++]  = QPoint (rect.right(), rect.height()*0.25);
         polygon[p++]  = polygon[0];            // close loop
         painter.drawPolygon (polygon, p);

         p = 0;
         polygon[p++]  = QPoint(rect.right(), rect.bottom());
         polygon[p++]  = QPoint(rect.width()*0.75, rect.bottom());
         polygon[p++]  = QPoint(rect.width()*0.75, rect.bottom()-cornerWidth);
         polygon[p++]  = QPoint (rect.right()-cornerWidth, rect.bottom()-cornerWidth);
         polygon[p++]  = QPoint (rect.right()-cornerWidth, rect.height()*0.75);
         polygon[p++]  = QPoint (rect.right(), rect.height()*0.75);
         polygon[p++]  = polygon[0];            // close loop
         painter.drawPolygon (polygon, p);
		}
#endif
...
void QSimpleShape::setCornerWidth (const int cornerWidthIn)
{
   this->cornerWidth = LIMIT (cornerWidthIn, 0, 20);
   this->update ();
}

//------------------------------------------------------------------------------
//
int QSimpleShape::getCornerWidth () const
{
