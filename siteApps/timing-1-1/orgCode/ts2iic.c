...
static STATUS iic_setMux(int fd, s_ts2iic* piic)
{
	...
	unsigned short int Sel_Channel = 0x0;
	unsigned int Mux_Reg = 0x1;
...
STATUS ts2iic_setCpsConfig(int busId, struct s_cpsConfig* pCfg)
{
	...
	txReg =0x10; txBuf = pCfg->eqBoost0;
	bytesWrite = iic_wr(busId, SLV_ADN4604, txReg, 1, (char*)&txBuf, 1 );
	if(bytesWrite != 1)
    prnErrRet();

	txReg =0x11; txBuf = pCfg->eqBoost1;
	bytesWrite = iic_wr(busId, SLV_ADN4604, txReg, 1, (char*)&txBuf, 1 );
	if(bytesWrite != 1)
    prnErrRet();
...
STATUS ts2iic_initClk(int option)
{
  ...
  prnM2("si5338_init Write new configuration... %d !!!!!!!!!!\r\n",option);
