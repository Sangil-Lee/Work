...
static int ocrDebug = 2;
epicsExportAddress(int, ocrDebug);

ocrAsynEpics::ocrAsynEpics(const char *portName, const char* filename, const char* devName, const double samprate, const int nofe)
	: asynPortDriver(portName, 
			...
			),fileName(filename),driverName("ocrAsynEpics"),system_init_ok(1),samplerate(samprate),element(nofe)
{
	printf("GCC Version-(%d)\n", GCC_VERSION);
    asynStatus status;
    const char *functionName = "ocrAsynEpics";
	...
	if (system_init_ok==1) 
	{
		status = (asynStatus)(epicsThreadCreate("ocrAsynEpicsUserTask",
...
ocrAsynEpics::~ocrAsynEpics()
{
}

void ocrAsynEpics::registerParamListFromFile(string filename)
{
...
asynParamType ocrAsynEpics::getAsynParamType(const char *paramstring)
{
...
asynStatus ocrAsynEpics::createParamNMap(RegMap &regmap)
{
...
epicsInt32 ocrAsynEpics::readInt32Value(const RegMap &regmap)
{
...
asynStatus ocrAsynEpics::readValue(const RegMap &regmap, epicsInt32 &value)
{
...
asynStatus ocrAsynEpics::readValue(const RegMap &regmap, epicsFloat64 &value)
{
	//Register address read.
	//....
...
asynStatus ocrAsynEpics::readFloat64Array(asynUser *pasynUser, epicsFloat64 *value, size_t nElements, size_t *nIn)
{
...
asynStatus ocrAsynEpics::readInt32(asynUser *pasynUser, epicsInt32 *value)
{
    int function = pasynUser->reason;
    int addr=0;
    asynStatus status = asynSuccess;
...
asynStatus ocrAsynEpics::readFloat64(asynUser *pasynUser, epicsFloat64 *value)
{
    int function = pasynUser->reason;
    int addr=0;
    asynStatus status = asynSuccess;
	...
	//asynStatus ocrAsynEpics::getParamValue(const string drvname, double &value)
	//
    /* Set the timestamp */
    pasynUser->timestamp = timeStamp;
...
asynStatus ocrAsynEpics::writeInt32(asynUser *pasynUser, epicsInt32 value)
{
	int function = pasynUser->reason;
	asynStatus status = asynSuccess;
...
asynStatus ocrAsynEpics::writeUInt32Digital(asynUser *pasynUser, epicsUInt32 value, epicsUInt32 mask)
{
    int function = pasynUser->reason;
    int addr=0;
    asynStatus status = asynSuccess;
...
int ocrAsynEpics::checkParam(const string drvname)
{
	int checkval = 0;
...
int ocrAsynEpics::setParamValue(const string drvname, const int ival)
{
...
int ocrAsynEpics::setParamValue(const string drvname, const double dval)
{
	int check = checkParam(drvname);
	if(check == 0)
...
int ocrAsynEpics::setParamValue(const string drvname, const string svalue)
{
	int check = checkParam(drvname);
	if(check == 0)
...
asynStatus ocrAsynEpics::getParamValue(const string drvname, int maxChars, char *value )
{
    asynStatus status = asynSuccess;
...
asynStatus ocrAsynEpics::getParamValue(const string drvname, int &value)
{
    asynStatus status = asynSuccess;
...
//===================laykim stringin
asynStatus ocrAsynEpics::readStringValue(const RegMap &regmap, char *value)
{
    asynStatus status = asynSuccess;
...
asynStatus ocrAsynEpics::readOctet(asynUser *pasynUser, char *value, size_t maxChars, size_t *nActual, int *eomReason)
{
    int function = pasynUser->reason;
    int addr=0;
    asynStatus status = asynSuccess;
...
asynStatus ocrAsynEpics::writeOctet(asynUser *pasynUser, const char *value, size_t maxChars, size_t *nActual)
{
    int function = pasynUser->reason;
	asynStatus status = asynSuccess;
...
asynStatus ocrAsynEpics::writeOctetValue(const RegMap &regmap, const char *value)
{
	asynStatus status = asynSuccess;
...
asynStatus ocrAsynEpics::writeStringValue(const RegMap &regmap, const char *strValue)
{
	asynStatus status = asynSuccess;
	if(checkParam(regmap.drvname) == -1) return(asynError);
...
int ocrAsynEpics::writeFloat64Value(const RegMap &regmap)
{
    int status = 0;
...
int ocrAsynEpics::writeInt32Value(const RegMap &regmap, epicsInt32 value)
{
	if(checkParam(regmap.drvname) == -1) return(-1);
...
//===================laykim longout
int ocrAsynEpics::writeInt32Value(const RegMap &regmap)
{
	int status = 0;
...
asynStatus ocrAsynEpics::writeFloat64(asynUser *pasynUser, epicsFloat64 value)
{
    int addr = 0;
    int function = pasynUser->reason;
    asynStatus status = asynSuccess;
...
void ocrAsynEpics::UserProcess()
{
	...
	mQtImage = QImage((const unsigned char*) (mFrame.data), mFrame.cols, mFrame.rows, QImage::Format_RGB888);
}

void ocrAsynEpics::getOptionBitset(OptionBit &optionbit, epicsUInt32 option, epicsUInt32 shift, epicsUInt32 mask)
{
	optionbit = (mask == 0) ? (option >> shift) : (option >> shift) & mask;
}

epicsUInt32 ocrAsynEpics::getRShiftMask(epicsUInt32 option, epicsUInt32 shift, epicsUInt32 mask = 0)
{
	return (mask == 0) ? (option >> shift) : (option >> shift) & mask;
}

epicsUInt32 ocrAsynEpics::getLShiftMask(epicsUInt32 option, epicsUInt32 shift, epicsUInt32 mask)
{
	return (mask == 0) ? (option << shift) : (mask << shift) | option;
}

epicsUInt32 ocrAsynEpics::setBitOnOff(epicsUInt32 option, epicsUInt32 nbit, bool bit)
{
	epicsUInt32 retVal = (bit==1) ? option |= 1 << nbit : option &= ~(1 << nbit);
	return (retVal);
}

epicsUInt32 ocrAsynEpics::setExclusiveBit(epicsUInt32 option, epicsUInt32 nbit)
{
	return (option ^= 1 << nbit);
}

bool ocrAsynEpics::checkBit(epicsUInt32 option, epicsUInt32 nbit)
{
...
{
	ocrAsynEpics *pPvt = (ocrAsynEpics *)drvPvt;
 ...
 */
epicsShareFunc int ocrAsynEpicsConfigure(const char *portName, const char* registerfile, const char *devName, const int samprate, const int nofe)
{
    new ocrAsynEpics(portName, registerfile, devName, samprate, nofe);
    return(asynSuccess);
}
...
static const iocshFuncDef initFuncDef = {"ocrAsynEpicsConfigure",5,initArgs};
static void initCallFunc(const iocshArgBuf *args)
{
    ocrAsynEpicsConfigure(args[0].sval, args[1].sval, args[2].sval,args[3].dval, args[4].ival);
}

void ocrAsynEpicsRegister(void)
{
    iocshRegister(&initFuncDef,initCallFunc);
}

epicsExportRegistrar(ocrAsynEpicsRegister);
}//end extern "C"
