...
paddinglist = ["13",   "13",         "10",   "18",              "18",               "33",   "33",   "10"]
...
pvname = ''
for e in signal:
    e = '{message: <{padcnt}}'.format(message = e+',', padcnt=int(15))
    pvname = pvname + e
text = '{'+pvname
sub.write(text)
...
for cnt in range(len(pvlist)):
    ...
    for orgidx, value in enumerate(valuelist):
        ...
        if(idx == len(valuelist)):
            ...
            #if(fieldlist[orgidx].find('INP') != -1):
            if(fieldlist[orgidx] == 'INPA'):
                pvname = inplist[0]+':Press'
                sncText = "\""+ pvname + value + "\""
            elif(fieldlist[orgidx] == 'INPB'):
                pvname = inplist[1]+':Temp'
                sncText = "\""+ pvname + value + "\""
            ...
            if(fieldlist[orgidx] == 'INPA'):
                ...
                sncText = "\"" + value+'\",'
            sncText = '{message: <{padcnt}}'.format(message = sncText, padcnt=int(str(paddinglist[idx-1])))
        sub.write(sncText)
    sncText = '}\n'
    sub.write(sncText)
