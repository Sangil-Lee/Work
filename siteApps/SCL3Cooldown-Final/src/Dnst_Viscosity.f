...
C Code, 3 = Density, 25 = Viscosity
C JOUT, 10000 for Density, 100 for Viscoty
      Real*8 function Dnst_Viscosity(Code, Press, Temp)
	INTEGER Code
	REAL*8 Press, Temp 
...
C	   PRINT *,'In Fortran:','Code:',Code,'Press:',Press,'Temp:',Temp
	   Dnst_Vsicosity = PROP(Code,0)
C       CALL ERRMSG (MESSAG, IDID)
C	   PRINT *,'Enthalpy:',Dnst_Vsicosity,'MSG',MESSAG
C
C Print selected results after a successful calculation for test. 
C 
C         WRITE (1, 10) Dnst_Vsicosity, IDID
      ELSE
C
