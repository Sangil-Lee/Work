...
*    ContLVDTSamps-IntClk-SCXI1540.c
*
* Example Category:
*    AI
*
...
*       secondary LVDT channels via the Synchronization Enabled
*       button. You must also connect the excitation output (EX+) of
*       your primary LVDT channel to all the secondary LVDT channel's
*       sync pin (SYNC).
*
* Steps:
*    1. Create a task.
...
*    Connect your LVDT to the terminals corresponding to the Physical
*    Channel I/O Control value. The excitation lines connect to EX+
*    and EX- while the analog input lines connect to CH+ and CH-
*
*********************************************************************/
...
int main(void)
{
	...
	DAQmxErrChk (DAQmxCreateAIPosLVDTChan(taskHandle,"Dev1/ai0","",-0.10,0.10,DAQmx_Val_Meters,50,DAQmx_Val_mVoltsPerVoltPerMillimeter,DAQmx_Val_Internal,1.0,2500,DAQmx_Val_4Wire,NULL));
