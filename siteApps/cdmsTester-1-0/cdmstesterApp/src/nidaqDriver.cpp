...
int32 CVICALLBACK DoneCallback(TaskHandle taskHandle, int32 status, void *callbackData);
	...
	nidaqDriver::nidaqDriver(const string device, const float64 minVal, const float64 maxVal, const float64 slope, 
			const float64 offset, const unsigned int scale, const int32 moduletype, const int32 termConfig, const float64 sampRate):software(0.1),g_taskHandle(0)
	{
		/*********************************************/
		...
		if(moduletype == 0)
		{
			if(scale == nidaqDriver::LinearScale)
			{
			...
			else if(scale == nidaqDriver::RangeMap)
			{
				createAIVoltChannel(device, minVal, maxVal,DAQmx_Val_RSE, DAQmx_Val_FromCustomScale, "UserRangeMap");
				//Default Sampling Clock Timing.
				configSampleClock();
			};
		}
	...
	nidaqDriver::nidaqDriver(const string device, float64 scaleparameter[],
			  const unsigned int scale, const int32 moduletype, const int32 termConfig,const float64 samplRate):software(0.1),g_taskHandle(0)
	{
		/*********************************************/
		// DAQmx Configure Code
		/*********************************************/
		DAQmxCreateTask("",&g_taskHandle);
		printf("scalemode(%d),type(%d),param[0]:%f,param[1]:%f,param[2]:%f,param[3]:%f\n", scale,moduletype,
				scaleparameter[0], scaleparameter[1], scaleparameter[2], scaleparameter[3]);
		...
		if(moduletype == 0)
		{
			if(scale == nidaqDriver::LinearScale)
			{
				...
				configSampleClock();
				setScale(nidaqDriver::LinearScale, "UserLineScale", scaleparameter, 2, DAQmx_Val_Volts, "bar");
			}
			else if(scale == nidaqDriver::RangeMap)
			{
				// 0 ~ 10v to 0 ~ 2bar for map scale.
				//createAIVoltChannel(device, 0.0, 2.0, DAQmx_Val_RSE, DAQmx_Val_FromCustomScale, "UserRangeMap");
				createAIVoltChannel(device, scaleparameter[2],scaleparameter[3],DAQmx_Val_RSE, DAQmx_Val_FromCustomScale, "UserRangeMap");

				//Default Sampling Clock Timing.
				configSampleClock();
				setScale(nidaqDriver::RangeMap, "UserRangeMap", scaleparameter, 4, DAQmx_Val_Volts, "bar");
			};
		}
		else if(moduletype == 1 )
		{
	...
	int nidaqDriver::setScale(e_scaleType scaletype, const string userScaleName, const float64 param[], const int paramcnt, const int32 prescaleunit, const string scaledunit)
	{
		//y = mx + b, (y=scaled value, x=raw value(0 ~ 10 volts), m = slope, b = y intercept (offset))
		//min ~ max (if m = 10, b = 5 -> max( y = 10 x 10 = 100), min (y = 0 x 10 = 0), b = not realated for min/max value)
		//CreateAIVoltChannel(device, termConfig, min_val, max_val, DAQmx_Val_FromCustomScale, "UserLineScale");
		switch (scaletype)
		{
			...
			case LinearScale:
				if(paramcnt == 2)
				{
					printf("paramcnt(%d), param[2]:%f,param[3]:%f\n", paramcnt, param[2], param[3]);
			...
			case RangeMap:
				...
				if(paramcnt == 4)
				{
					printf("paramcnt(%d), param[0]:%f,param[1]:%f,param[2]:%f,param[3]:%f\n", paramcnt,
							param[0], param[1], param[2], param[3]);
					DAQmxCreateMapScale(userScaleName.c_str(),param[0], param[1],param[2],param[3],prescaleunit, scaledunit.c_str());
					//DAQmxCreateMapScale(userScaleName.c_str(), 0.0, 10.0, 0.0, 2.0, prescaleunit, scaledunit.c_str());
				};
				break;
			case Polynomial:
				{
