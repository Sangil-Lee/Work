...
STATUS ts2ip_wr(int fd, unsigned int offset, unsigned int wrData)
{
  if (fd == -1) 
		prnErrRet();
...
static STATUS ts2ip_getInfo(int l_fd, struct s_ipInfo *pInfo)
{
  ...
  int rdData = 0;
	ts2ip_rd(l_fd, A_vendor, (unsigned int*)&rdData);	
	pInfo->vendor = rdData;

	ts2ip_rd(l_fd, A_ipInfo, (unsigned int*)&rdData);
  ...
  pInfo->ipVer_rev   = S_ipVer_rev(rdData); 
	...
	ts2ip_rd(l_fd, A_buildTime, (unsigned int*)&rdData);
...
STATUS ts2ip_getGtpStat(struct s_ts2ip* pIP)
{
  ...
  memset(pStat,0,sizeof(struct s_gtpStat));
	...
	ts2ip_rd(l_fd, A_gt_status, (unsigned int*)&rdData);	
  ...
  pStat->gt1_trackOK          = S_gt1_trackOK(rdData)      ;
	...
	ts2ip_rd(l_fd, A_gt_trackLossCnt, (unsigned int*)&rdData);	
  pStat->gt0_trackLossCnt     = S_gt0_trackLossCnt(rdData)    ;
  pStat->gt1_trackLossCnt     = S_gt0_trackLossCnt(rdData)    ;

	ts2ip_rd(l_fd, A_gt0_clk_cntr, (unsigned int*)&rdData);	
  pStat->gt0_txClk_cntr       = S_gt0_rxClk_cntr(rdData)      ;
  pStat->gt0_rxClk_cntr       = S_gt0_txClk_cntr(rdData)      ;

	ts2ip_rd(l_fd, A_gt1_clk_cntr, (unsigned int*)&rdData);	
...
STATUS ts2ip_getEvrTime(struct s_ts2ip* pIP)
{
  int rdData = 0;
	...
	ts2ip_rd(l_fd, A_evrW_Time, (unsigned int*)&rdData);
  ...
  pStat->evrTime.ms   = S_evrW_TimeMs(rdData);  
	...
	ts2ip_rd(l_fd, A_evrW_Day, (unsigned int*)&rdData);
...
STATUS ts2ip_getEvStat(struct s_ts2ip* pIP)
{
  ...
  ts2ip_rd(l_fd, A_boardStat, (unsigned int*)&rdData);
  ...
  ts2ip_rd(l_fd, A_ifanStt4_CntrFront, (unsigned int*)&rdData);
	pStat->fanA = S_ifanStt4_CntrFrontA(rdData);
	pStat->fanB = S_ifanStt4_CntrFrontB(rdData);

  ts2ip_rd(l_fd, A_ifanStt4_CntrRear, (unsigned int*)&rdData);
	pStat->fanC = S_ifanStt4_CntrRearA(rdData);
	pStat->fanD = S_ifanStt4_CntrRearB(rdData);

  ts2ip_rd(l_fd, A_emergencyStopCntr, (unsigned int*)&rdData);
	pStat->emergencyStopCntr = rdData;

  ts2ip_rd(l_fd, A_seqTrg_runState, (unsigned int*)&rdData);
	pStat->seqA_isRunning = S_seqTrgA_runState(rdData);
	pStat->seqB_isRunning = S_seqTrgA_runState(rdData);

	ts2ip_rd(l_fd, A_pdpRunStt, (unsigned int*)&rdData);
	for(i=0;i<32;i++)
	...
	for(i=0;i<32;i++)
	{
		ts2ip_rd(l_fd, MemC_EVR_pdpStat + i*4, (unsigned int*)&rdData);
