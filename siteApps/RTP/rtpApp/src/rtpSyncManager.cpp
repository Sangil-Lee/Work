...
*/
	...
	switch(type)
	{
		case BOOL_WRITE:
			{
				...
				check = getCRC((unsigned char*)writeMsg, sendByte);
				writeMsg[sendByte++]= check & 0xFF; 
				writeMsg[sendByte++]= (check >> 8) & 0xFF; 
			}
			break;
		case INT_WRITE:	
			{
		...
		default:
			{
				printf("FVAlue=====>: %f\n", fvalue);
				memcpy(&writeMsg[sendByte++], &fvalue, sizeof(epicsFloat32));
				//unsigned short check = getCRC((unsigned char*)sWCommand, SINGLE_CRC_WFLOAT_INDEX);
				//msgSize = SINGLE_CRC_WFLOAT_INsendByteDEX + 2;
				check = getCRC((unsigned char*)writeMsg, sendByte);
				writeMsg[sendByte++]= check & 0xFF; 
				writeMsg[sendByte++]= (check >> 8) & 0xFF; 
	...
	return send(mptty->fd, (const char*)sWCommand, sendByte, 0);
