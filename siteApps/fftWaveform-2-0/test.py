...
class TestFFTW(unittest.TestCase):
    ...
    def test_1ke_asub_1sine(self):
        ...
        data = amplitude*sizetimes*np.cos(Phase1*2 * np.pi * np.arange(arrsize) / arrsize) + amplitude2*sizetimes*np.sin(Phase2 * 2 * np.pi * np.arange(arrsize) / arrsize)
