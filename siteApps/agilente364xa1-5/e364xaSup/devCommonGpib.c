...
#ifdef AGILENT_E3632A
			char skipChars[39];
			char realChars[39];
			sscanf(pgpibDpvt->msg, "%39[^+]%s", skipChars,realChars); 
#ifdef DEBUG
			asynPrint(pasynUser,ASYN_TRACE_ERROR,"msg(%s),%s,%s\n", pgpibDpvt->msg, skipChars, realChars);
#endif
            result = sscanf(realChars, format, &value);
#else
...
static void biFinish(gpibDpvt * pgpibDpvt,int failure)
{
    ...
    if(failure) {; /*do nothing*/
        ...
        if(sscanf(pgpibDpvt->msg, format, &value) == 1) {
            ...
            failure = -1;
            asynPrint(pasynUser,ASYN_TRACE_ERROR,"msg(%s),format(%s)\n",
                format, pgpibDpvt->msg);
