C This is a program to calculate the density of LHe in the dewar.
C The following assignments are required.
C
      Real*8 function DnstL5301(Press)
			REAL*8 Press
...
C
C Note: PROP(3,j) is the density of LHe in the dewar. 
C The unit is "g/L".
C
C------------------------------------- OUTPUT DATA					   
C
...
C the output data is inputted into the result variable, "DnstL5301".
C
	   DnstL5301 = PROP(3,0)
C
C Print selected results after a successful calculation for test. 
C 
C         WRITE (1, 10) DnstL5301, IDID
      ELSE
C
...
C the DnstL5301 will be zero for EPICS to notice the failure.
C
	   DnstL5301 = 0
      ENDIF
