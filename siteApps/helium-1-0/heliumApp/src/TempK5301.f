C This is a program to calculate the temperature of LHe in the dewar.
C The following assignments are required.
C
      Real*8 function TempK5301(Press)
			REAL*8 Press
...
C JIN1 is an input variable for the saturated liquid.
C JIN2 is an input variable for the pressure of LHe in the dewar.
...
C
C Note: PROP(2,j) is the temperature of LHe in the dewar. 
C
...
C the output data is inputted into the result variable, "TempK5301".
C
	   TempK5301 = PROP(2,0)
C
C Print selected results after a successful calculation for test. 
C 
C         WRITE (1, 10) TempK5301, IDID
      ELSE
C
...
C the TempK5301 will be zero for EPICS to notice the failure.
C
	   TempK5301 = 0
      ENDIF
