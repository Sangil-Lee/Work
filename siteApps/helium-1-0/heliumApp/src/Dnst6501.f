C This is a program to calculate the density of GHe in the buffer tanks.
C The following assignments are required.
C
      Real*8 function Dnst6501(Temp, Press)
			REAL*8 Temp, Press
...
C		PRINT *,'In Fortran:','Temp:',Temp,'Press:',Press
C    
...
C JIN1 is an input variable for the temperature of GHe in the buffer tanks.
C JIN2 is an input variable for the pressure of GHe in the buffer tanks.
...
C the output data is inputted into the result variable, "Dnst6501".
C
	   Dnst6501 = PROP(3,0)
C
C Print selected results after a successful calculation for test. 
C 
C         WRITE (1, 10) Dnst6501, IDID
      ELSE
C
...
C the Dnst6501 will be zero for EPICS to notice the failure.
C
	   Dnst6501 = 0
      ENDIF
