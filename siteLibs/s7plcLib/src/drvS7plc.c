...
int s7plcConfigure(char *name, char* IPaddr, int port, int inSize, int outSize, int bigEndian, int recvTimeout, int sendIntervall)
{
    ...
    if (!name)
    {
        errlogSevPrintf(errlogFatal, "s7plcConfigure: missing name\n");
        return -1;
    }
    if (!IPaddr)
    {
        errlogSevPrintf(errlogFatal, "s7plcConfigure: missing IP address\n");
        return -1;
    }
    if (!port)
    {
        errlogSevPrintf(errlogFatal, "s7plcConfigure: missing IP port\n");
        return -1;
    }
    ip = inet_addr(IPaddr);
    if (ip == INADDR_NONE)
    {
        errlogSevPrintf(errlogFatal, "s7plcConfigure: invalid IP address %s\n", IPaddr);
        return -1;
    }
    ip = ntohl(ip);
...
s7plcStation *s7plcOpen(char *name)
{
    ...
    errlogSevPrintf(errlogFatal, "s7plcOpen: station %s not found\n", name);
    return NULL;
}
