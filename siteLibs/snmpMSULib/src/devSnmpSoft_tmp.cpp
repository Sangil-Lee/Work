  ...
  if (pPV->getValueDouble(&new_val)) 
{
	...
	sprintf(sval, "%f", new_val); 
	strcpy(psnmp->val, sval);
...
{
    //char*strcpy(const char*string_val,const char*val);
    status = 2;
    ...
    {
      if (strcmp(string_val,psnmp->val)!=0) 

	{
        // value read from remote host doesn't match what we think val is
        if (snmpDebugLevel)
          printf("----- devSnmp Soft value change %s [%s] -> [%s] ...\n",psnmp->name,psnmp->val,string_val);
        strcpy(psnmp->val,string_val);
        process_record = true;
  	} 
    }
else 
