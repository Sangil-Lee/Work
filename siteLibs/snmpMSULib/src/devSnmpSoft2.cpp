...
//--------------------------------------------------------------------
  // static epicsStatus snmpWFSoftInit(struct snmpWFRecord *psnmpWF);
  // static epicsStatus snmpWFSoftRead(struct snmpWFRecord *psnmpWF);
}
...
//   DEVSUPFUN       init_record;
//   DEVSUPFUN       ioIntInfoGet;
//   DEVSUPFUN       read_snmpWF;
//   DEVSUPFUN       special_linconv;
// } devSnmpWFSoft = {
//   6,
//   NULL,
//   NULL,
//   (DEVSUPFUN) snmpWFSoftInit,
//   NULL,
//   (DEVSUPFUN) snmpWFSoftRead,
//   NULL
// };
// epicsExportAddress(dset,devSnmpWFSoft);
...
//   switch (psnmpWF->inp.type) {
//   case INST_IO:
//     break;

//   default :
//     recGblRecordError(S_db_badField,(void *)psnmpWF,
//                       "devSnmpSoftWf (init_record) Illegal INP field");
//     return(S_db_badField);
//   }

//   if (! checkInit()) return(epicsError);
//   devSnmp_pv *pPV = pManager->addPV((struct dbCommon *) psnmpWF, &psnmpWF->inp);
//   if (! pPV) {
//     recGblRecordError(S_db_badField,
//                       (void *)psnmpWF,"devSnmpSoftWf (init_record) bad parameters");
//     return(S_db_badField);
//   }
...
//     case DBF_CHAR:
//     case DBF_UCHAR:
//         pPV->setDataLength((sizeof(char) * psnmpWF->nelm) + 1);
//         break;
//     default:
...
//   if (psnmpWF->dpvt == NULL)
//     return(epicsError);
//   else
//     pPV = (devSnmp_pv *) psnmpWF->dpvt;
...
//         if (!wfTypes[i].func(psnmpWF->bptr, (char *)val)) {
//           psnmpWF->nord = 1;
// #if     1
//           switch (psnmpWF->ftvl) {
//             case DBF_CHAR:
//             case DBF_UCHAR:
//             case DBF_STRING:
//                 psnmpWF->nord = strlen((char *)psnmpWF->bptr);
//                 break;
// #if     0
//             case DBF_CHAR:
//             case DBF_UCHAR:
//                 printf("bptr char %c\n", psnmpWF->bptr,0,0,0,0,0);
//                 break;
...
//     }
//   } else {
//     status = recGblSetSevr(psnmpWF, READ_ALARM, INVALID_ALARM);
//     if (status && (psnmpWF->stat != READ_ALARM || psnmpWF->sevr != INVALID_ALARM)) {
//       if (snmpDebugLevel) printf("%s  devSnmp WF read error '%s' : %s\n",tnow(),psnmpWF->name,pPV->errorString());
//     }
//     status = 2;
//   }
