...
#ifdef DEBUG
	printf ("==========>Set timeout to %i.%06i seconds\n",
			(int) timeout.tv_sec,
			(int) timeout.tv_usec);
#endif
...
#ifdef DEBUG
		printf ("==========>Waiting on %i sockets for %i.%06i seconds\n", num_fds,
				(int) timeout.tv_sec,
				(int) timeout.tv_usec);
#endif
...
#ifdef DEBUG
	//printf("FD(%d), Buf(%p), Buflen(%d), Addr(%p),Addrlen(%s)\n",ph->fd, buf,(int)buflen, ph->addr,(int)ph->addrlen);
#endif
...
static int ping_send_one_ipv4 (pingobj_t *obj, pinghost_t *ph)
{
	...
	*icmp4 = (struct icmp) {
	...
	if (sizeof (buf) < buflen) return (EINVAL);
