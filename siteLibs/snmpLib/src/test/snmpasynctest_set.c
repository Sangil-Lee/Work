 ...
 * Wednesday, January 14 18:27:08 KST 2015 (snmpset done v0.1) thanks to SL
 */
...
//version 2c
  ...
  if (!read_objid(psnmp->oids, Oid, &OidLen))      /* parse the oids */
    {
      printf("parse     %s\n",psnmp->oids);
  ...
  req = snmp_pdu_create(SNMP_MSG_SET);	/* send the first SET */

  char sval[6];

  sprintf(sval, "%d", psnmp->rval);   //silee

  //snmp_add_var(req, Oid, OidLen, type, &psnmp->rval);
  snmp_add_var(req, Oid, OidLen, type, sval);

  /* printf("rval : %d  &rval: %d \n", psnmp->rval, &psnmp->rval ); */


  if (snmp_send(snmpinfo->sess, req))
    active_hosts++;
  else {
