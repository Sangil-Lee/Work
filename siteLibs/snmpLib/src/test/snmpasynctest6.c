...
void actve_hosts();
void asynchronous_get(struct snmpRecord *psnmp);
void asynchronous_set(struct snmpRecord *psnmp);
int return_getdata(int status, struct snmp_session *sp, struct snmp_pdu *pdu);
int return_setdata(int status, struct snmp_session *sp, struct snmp_pdu *pdu);
int asynch_response_get(int operation, struct snmp_session *sp, int reqid, struct snmp_pdu *pdu, void *magic);
int asynch_response_set(int operation, struct snmp_session *sp, int reqid, struct snmp_pdu *pdu, void *magic);

static long snmp_Init(struct snmpRecord *psnmp);
static long snmp_Readback(struct snmpRecord *psnmp);
...
switch (status) {
  ...
  case STAT_SUCCESS:
	...
	/*mjpark------------------------------------------*/
	setdata = (char *)malloc(1 + vp->val_len);
	memcpy(setdata, vp->val.string, vp->val_len);
	setdata[vp->val_len] = '\0';
	int nVal;
	memcpy((void *)&nVal, setdata, 4);
	gpSnmp->val = nVal;
...
int asynch_response_get(int operation, struct snmp_session *sp, int reqid, struct snmp_pdu *pdu, void *magic)
{
  ...
  if ((operation == NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE)) {
    return_getdata(STAT_SUCCESS, snmpinfo->sess, pdu); 

  } else {
    return_getdata(STAT_TIMEOUT, snmpinfo->sess, pdu);   
...
int asynch_response_set(int operation, struct snmp_session *sp, int reqid, struct snmp_pdu *pdu, void *magic)
{
  ...
  if ((operation == NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE)) {
    return_setdata(STAT_SUCCESS, snmpinfo->sess, pdu); 

  } else {
    return_setdata(STAT_TIMEOUT, snmpinfo->sess, pdu);
...
void asynchronous_get(struct snmpRecord *psnmp)
{
  ...
  sess.callback = asynch_response_get;		/* default callback */
...
void asynchronous_set(struct snmpRecord *psnmp)
{
  ...
  sess.callback = asynch_response_set;		/* default callback */
  sess.callback_magic = snmpinfo;
  ...
  sprintf(snmpinfo->sval, "%d", psnmp->rval);   //silee
