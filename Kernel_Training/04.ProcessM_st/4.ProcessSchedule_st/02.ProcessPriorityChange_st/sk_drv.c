...
ssize_t sk_read (struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	//printk(KERN_WARNING "[SK_DD]read(pid:%d(%s), prio:%d, static_prio:%d, sleep_avg:%lx)..\n", current->pid, current->comm, current->prio, current->static_prio, current->sleep_avg);
	...
	//printk(KERN_WARNING "[SK_DD]read(pid:%d(%s), sleep_max:%llu, sum_sleep_runtime:%llu, slice_max:%llu)..\n", current->pid, current->comm, current->se.sleep_max, current->se.sum_sleep_runtime, current->se.slice_max);
#ifdef HOST
...
#else
	printk(KERN_WARNING "[SK_DD]read(pid:%d(%s), sleep_max:%llu, slice_max:%llu, vruntime:%llu)..\n", current->pid, current->comm, current->se.sleep_max, current->se.slice_max, current->se.vruntime);
#endif
	//printk(KERN_WARNING "[SK_DD]read(pid:%d(%s), rt_timeslice:%u)..\n", current->pid, current->comm, current->rt.time_slice);
