...
DECLARE_WAIT_QUEUE_HEAD(wq_led);
...
irqreturn_t timer0_int(int irq, void *dev_id)
{
	...
	printk("[LED_DD]Timer0 Int!!!!\n");

#if 0
	tcon = __raw_readl(S5P_VIC0REG(VIC_INT_ENABLE_CLEAR));
	tcon |= (1<<21);	//Timer0 Int. Pending Clear
	__raw_writel(tcon, S5P_VIC0REG(VIC_INT_ENABLE_CLEAR));
#endif
...
ssize_t led_write (struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
    ...
    printk("Timer0 Int. Waiting(%d)...\n", flag_timer0Int);
	//while(!flag_timer0Int);
    if(!flag_timer0Int) {
        if(!(filp->f_flags & O_NONBLOCK)) {
            //interruptible_sleep_on(&wq_led);
			ret = wait_event_interruptible(wq_led, flag_timer0Int);
            
            printk("[LED_DD]LED_Write_WakeUp(ret:%d)!!\n", ret);
			if(ret<0) return -1;
        } else {
            return -EAGAIN;
        }
...
int led_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	printk(KERN_WARNING "[LED_DD]ioctl(cmd:%x,arg:%ld)..\n", cmd, arg);
...
unsigned int led_poll(struct file *filp, poll_table *wait)
{
	unsigned int mask = 0;
	........... 
