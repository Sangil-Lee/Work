...
//Buddy: Page(4K:32bit, 8K:64bit) memory to physical address, 
//Slab argolrithm: Divide in Page  
...
{
	//__user : in kernel code, user space address
	int k_data;
	int ret;
...
ssize_t sk_write (struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	//k_data = *((int*)buf); , Can't do this, because of __user --> 
