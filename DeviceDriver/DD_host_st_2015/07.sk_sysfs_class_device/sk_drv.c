...
int sk_init(void)
{
	...
	sk_class = __________(THIS_MODULE, "sk");
	if(IS_ERR(sk_class)) {
	...
	sk_c_dev.______ = sk_class;
	sk_c_dev._____ = sk_dev;
	result = __________(&sk_c_dev);
	if(result < 0) {
		printk(KERN_WARNING "Class_Device(sk_c_dev):Register Fail(%d)!!\n", result);
		cdev_del(&sk_cdev);
		unregister_chrdev_region(sk_dev, 1);
		class_destroy(sk_class);
		return result;
	}
	printk(KERN_WARNING "Class_Device(sk_c_dev):Register OK!!\n");
