...
class ServerModel
{
	...
	try {
		fos = new FileOutputStream("/home/ctrluser/alarm-server_log.txt");
	} catch(IOException e) {
	        e.printStackTrace();
	}
...
*/
	...
	//silee++
        try
        {
            ...
            //logger.info("====>sendStateUpdate >>>>>>> count:" + (count++));
	    
	    String skey = record.key();
	    if(skey.matches("(.*)Alarm10(.*)"))
	    {
		//logger.info("Time:(" + dateformat.format(new Date()) + "), Count:("+(count++)+"), Key: " + skey + "Json: " + record.value());
		final JsonNode jsonNode  = (JsonNode) JsonModelReader.parseJsonText(record.value());
		final JsonNode timeNode  = jsonNode.get(JsonTags.TIME);
		final JsonNode secNode   = timeNode.get(JsonTags.SECONDS);
		final JsonNode nanoNode  = timeNode.get(JsonTags.NANO);

		final long lsec = secNode.asLong(); 
		final long lnano = nanoNode.asLong();

		final Instant epoch = Instant.ofEpochSecond(lsec).plusNanos(lnano); 

		final String strsec = TimestampFormats.MILLI_FORMAT.format(epoch); 
		final String logstr = "Time:(" + dateformat.format(new Date()) +", A-Time:"+strsec+"), Count:("+String.valueOf(count++)+"), Key: " + skey + ", Json-format: " + record.value()+"\n";

		/*
		final String nano = nanoNode.asText().substring(0, 3);
		final String strsec = TimestampFormats.SECONDS_FORMAT.format(epoch); 
		final String logstr = "Time:(" + dateformat.format(new Date()) +", A-Time:"+strsec+"."+nano+ "), Count:("+String.valueOf(count++)+"), Key: " + skey + ", Json-format: " + record.value()+"\n";
		*/
        ...
        catch (Throwable ex)
        {
            ...
            logger.info("Cannot send state update for " + path + ex);
        }
    }
        ...
        if (now - last_state_update  >  AlarmSystem.idle_timeout_ms)
	{
            ...
            logger.info("====>Server model thread >>>>>>> count:" + count++);
	};
    }
